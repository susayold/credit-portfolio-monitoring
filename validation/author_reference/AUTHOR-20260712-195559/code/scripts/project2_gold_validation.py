...
def _validate_reference_temporal_order(reference: dict[str, object]) -> dict[str, object]:
    ...
    current_code_matches_author = code_manifest_pass
    ...
    if code_manifest_pass:
        ...
        for row in code_manifest.get("files", []):
            ...
            current_code = ROOT / str(row.get("relative_path", ""))
            frozen_code = ROOT / str(row.get("frozen_code_path", ""))
            current_code_matches_author = current_code_matches_author and current_code.exists() and sha256(current_code) == expected_code_hash
    ...
    result = {
        ...
        "current_reviewer_code_matches_author": current_code_matches_author,
        ...
        "status": "PASS" if temporal_pass and bundle_pass and source_pass and code_manifest_pass and current_code_matches_author and frozen_code_matches_author and author_artifacts_pass and public_path_redaction_pass else "FAIL",
    }
    ...
    reference_controls = pd.DataFrame(
        [
            ...
            ["REF-010", "Reviewer code and frozen author code match exactly", current_code_matches_author and frozen_code_matches_author, True, current_code_matches_author and frozen_code_matches_author],
...
def create_executable_tests() -> None:
    ...
    dashboard_page_count = len(re.findall(r'<section\b[^>]*\bclass="[^"]*\bpage\b[^"]*"', dashboard_html))
    add(54, "Dashboard", "Eight executable dashboard views exist", dashboard_page_count, 8, dashboard_page_count == 8, "Exact HTML section-page selector")
