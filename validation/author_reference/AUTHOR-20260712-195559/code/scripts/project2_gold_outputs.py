...
def _dax_rows() -> list[list[str]]:
    return [
        ...
        ["Total Exposure", "SUM(fact_portfolio_snapshot[exposure])", "#,##0", "Exposure proxy", "outputs/portfolio_kpi_summary.csv"],
...
<div class="decision"><div class="eyebrow" style="color:var(--teal)">Management recommendation</div><b>{recommendation.replace('_',' ')}</b><p>The 20% cutoff candidate reduces straight-through auto-approved EL {_pct(float(candidate['auto_approved_el_reduction_vs_baseline']))}, but approval falls {_bps(candidate_approval_drop)} and manual-review volume rises {candidate_review_relative:.1%}. That is a routing benefit, not a realised portfolio-loss claim.</p><div class="decision-grid"><div><small>Proceed</small><strong>Constrained pilot</strong></div><div><small>Required control</small><strong>Capacity and stop criteria</strong></div><div><small>Do not claim</small><strong>Wholesale loss reduction</strong></div></div></div>
</div></section>
